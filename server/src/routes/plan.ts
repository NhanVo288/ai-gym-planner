...
planRoute.post("/generate", async (req: Request, res: Response) => {
  try {
    ...
    if (!profile) {
      return res.status(400).json({ error: "User profile is not found" });
    }
...
planRoute.get('/current',async(req: Request, res:Response) => {
    try {
        ...
        res.status(500).json({error: "Faild to fetch plan"})
    }
})
